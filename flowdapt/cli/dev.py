...
dev_cli = AsyncTyper(
    name="dev",
    short_help="Commands for development.",
    help=__doc__,
    hidden=True
)
...
async def spec(
    ...
    services: list[str] = typer.Option(
        ...
        "-s",
        "--service",
        help="The Service API specifications to generate."
    ),
    version: str = typer.Option(
        "latest",
        "-v",
        "--version",
        help="The specification version to generate."
    ),
    openapi_version: str = typer.Option(
        "3.0.2",
        "--openapi-version",
        help="The OpenAPI specificationv version to use."
    ),
    output_path: Path = typer.Option(
        "openapi.json",
        "-o",
        "--output",
        help="The path to write the specification file."
    ),
    json: bool = typer.Option(
        True,
        "--json/--yaml",
        help="The format to write the specification file."
    )
):
    """
    ...
    from flowdapt.lib.rpc.api import create_api_server, APIServer
