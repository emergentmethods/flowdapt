...
from flowdapt.lib.rpc.eventbus.event import (
    Event as Event,
    ResponseEvent as ResponseEvent,
    BaseEvent as BaseEvent,
    EndOfStream as EndOfStream
)
from flowdapt.lib.rpc.eventbus.callback import (
...
async def create_event_bus(
    ...
    **kwargs
    ...
    _AVAILABLE_BROKERS = {
        "memory": lambda: import_from_string("flowdapt.lib.rpc.eventbus.brokers.memory.MemoryBroker"),  # noqa
        "amqp": lambda: import_from_string("flowdapt.lib.rpc.eventbus.brokers.rabbitmq.RabbitMQBroker")  # noqa
    }
    ...
    kwargs.update(
        {
            "_serializer": _AVAILABLE_SERIALIZERS[serializer](),
            "url": url
        }
    )
