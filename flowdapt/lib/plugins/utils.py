...
from flowdapt.lib.utils.misc import in_path, get_python_executable
from flowdapt.lib.utils.asynctools import run_in_thread, call_bash_command


async def parse_package_manifest(
    manifest_path: Path,
    skip_words: list[str] = []
) -> list[Path]:
    """
    ...
    for line in content.splitlines():
        ...
        if line and not line.startswith("#"):
            ...
            match directive:
                case "include":
                    for pattern in args:
                        files_included.update(
                            manifest_dir.rglob(pattern)
                        )
                case "exclude":
                    for pattern in args:
    ...
    files_excluded.update(
        [
            path for path in files_included
            if in_path(path, skip_words)
        ],
        [
            path for path in files_included
            if await run_in_thread(path.is_dir)
        ]
    )
