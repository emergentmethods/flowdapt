...
from flowdapt.lib.utils.model import is_pydantic_model, BaseModel
...
def get_nested_field(obj: Any, fields: list) -> Any:
    ...
    for part in fields:
        if isinstance(part, int):
            ...
            obj = [
                item.get(part, None) if isinstance(item, dict) else None for item in obj
            ]
        else:
    ...
    for field_name, field_type in type_hints.items():
        ...
        if is_immutable_field(field_type):
            ...
            immutable_fields.update(
                find_immutable_fields(field_type, full_field_path)
            )
...
def merge(
    model: BaseModel,
    patch: dict,
    immutable_fields: dict[tuple, bool],
    base_path: tuple = ()
) -> None:
