...
async def list_triggers_api(
    versioned_dto: tuple[DTOPair, str] = Depends(
        get_versioned_dto(
            TriggerRuleReadDTOs,
            resource_type=TRIGGER_RESOURCE_KIND
        )
    )
):
...
@router.add_api_route(
    ...
    openapi_extra={
        "requestBody": {
            **requests_from_dtos(TriggerRuleCreateDTOs, TRIGGER_RESOURCE_KIND)
        }
    },
    status_code=status.HTTP_200_OK,
    name="create_trigger"
...
@router.add_api_route(
    ...
    openapi_extra={
        "requestBody": {
            **requests_from_dtos(TriggerRuleUpdateDTOs, TRIGGER_RESOURCE_KIND)
        }
    },
    status_code=status.HTTP_200_OK,
    name="update_trigger"
